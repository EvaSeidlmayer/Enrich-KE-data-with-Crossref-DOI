...
def main():
    ...
    with jsonlines.open(args.jsonl_file) as json:
        ...
        for identifier, data in tqdm(flat_gen):
            ...
            if data["sortyear"][0] not in year_filter:
                continue
            if "DOI" in data:
                # publication has already a DOI, drop record
                continue
            try:
                if data["DOCTYPE"][0] != 'ARTIKEL':
                    continue
            except:
                continue
            ...
            try:
                ...
                continue
            metadata.append((dbrecordid, otherID, authors, title, sortyear, publyear, publisher))
    ...
    dfs = [
        ...
        ('metadata_KE_no-DOI', df_paper_metadata, False)
    ]
